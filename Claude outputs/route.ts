...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      physioMuscular, physioNeuro, physioCellular, sportsMedicineConcepts,
    ] = await Promise.all([
      ...
      adminSupabase.from('physiology_concepts').select('id, name, category').eq('system', 'cellular').or(orName).limit(6),
      adminSupabase.from('sports_medicine_concepts').select('id, name, category').or(orName).limit(8),
    ]);
    ...
    push(physioCellular.data, 'phys-c', 'physiology', 'Fisiologia — Cellulare', '/dashboard/physiology?system=cellular');
    push(sportsMedicineConcepts.data, 'sm', 'sports-medicine', 'Medicina dello Sport', '/dashboard/sports-medicine');
