...
const PATHWAY_CATEGORY_LABEL: Record<string, string> = {
  'long-tracts': 'Long Tracts',
...
export default function BrainMapPage() {
  ...
  const [pathwayCategoryFilter, setPathwayCategoryFilter] = useState<'long-tracts' | 'brain-circuits'>('long-tracts');  const [nervesSubView, setNervesSubView] = useState<'atlas' | 'seddon' | 'conditions'>('atlas');
          ...
          <div>
            <div className="flex justify-center mb-8">
              <div className="inline-flex rounded-full border border-black/[0.06] dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.03] p-1">
                  ...
                  >
                    {sv === 'circuits' ? 'Pathways' : sv === 'gait' ? 'Gait Patterns' : 'Localization'}
                  </button>
                ))}
              </div>
            </div>
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-[11px] font-semibold transition-all ${
                        pathwayCategoryFilter === cat
                          ? 'text-white'
                          : 'text-ink/50 dark:text-white/50 border border-black/[0.08] dark:border-white/10 hover:text-ink dark:hover:text-white'
                      }`}
                      style={pathwayCategoryFilter === cat ? { background: '#32D6A0' } : undefined}
                    >
                      {PATHWAY_CATEGORY_LABEL[cat]}
                    </button>
                  ))}
                </div>
            ...
            {pathwaySubView === 'localization' && (
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {LOCALIZATION_PRINCIPLES.map((lp) => (
                    <div
                      key={lp.title}
                      className="rounded-2xl border border-black/[0.06] dark:border-white/10 bg-white/70 dark:bg-white/[0.03] backdrop-blur-xl p-5"
                    >
