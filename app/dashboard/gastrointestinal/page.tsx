...
export default function GastrointestinalPage() {
  ...
  useEffect(() => {
    ...
    const fetchStructures = async () => {
      ...
      try {
        const res = await fetch(`/api/gastrointestinal/structures?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle strutture');
        const data = await res.json();
        setStructures(data.structures ?? []);
        setHasFetchedStructures(true);
      } catch (err) {
        setStructuresError(ui.gastrointestinal.errorLoadingStructures);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchConditions = async () => {
      ...
      try {
        const res = await fetch(`/api/gastrointestinal/conditions?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle patologie');
        const data = await res.json();
        setConditions(data.conditions ?? []);
        setHasFetchedConditions(true);
      } catch (err) {
        setConditionsError(ui.gastrointestinal.errorLoadingConditions);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchTests = async () => {
      ...
      setTestsError(null);
      try {
        const res = await fetch(`/api/gastrointestinal/tests?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero dei test');
        const data = await res.json();
        setTests(data.tests ?? []);
        setHasFetchedTests(true);
      } catch (err) {
        setTestsError(ui.gastrointestinal.errorLoadingTests);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchRehab = async () => {
      ...
      setRehabError(null);
      try {
        const res = await fetch(`/api/gastrointestinal/rehab?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero dei contenuti riabilitativi');
        const data = await res.json();
        setRehab(data.rehab ?? []);
        setHasFetchedRehab(true);
      } catch (err) {
        setRehabError(ui.gastrointestinal.errorLoadingRehab);
        console.error(err);
      } finally {
  ...
  const SUB_TABS: { key: SubView; label: string }[] = [
    { key: 'anatomy', label: ui.gastrointestinal.subTabs.anatomy },
    { key: 'conditions', label: ui.gastrointestinal.subTabs.conditions },
    { key: 'assessment', label: ui.gastrointestinal.subTabs.assessment },
    { key: 'rehab', label: ui.gastrointestinal.subTabs.rehab },
  ];
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-[#08090b] text-ink dark:text-white overflow-hidden transition-colors">
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(245,158,11,0.6) 0%, rgba(34,197,94,0.5) 100%)',
        }}
      />
      ...
      <div className="relative max-w-4xl mx-auto px-6 pt-40 pb-24">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-black/[0.08] dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.03] backdrop-blur-xl text-xs font-semibold tracking-[0.15em] uppercase mb-4">
            ...
            {ui.gastrointestinal.atlasBadge}
          </div>
        ...
        {subView === 'anatomy' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.gastrointestinal.anatomyHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.gastrointestinal.anatomyHint}
            </p>
            ...
            <div className="mb-10 rounded-2xl border p-6" style={{ borderColor: `${ACCENT.solid}33`, background: `${ACCENT.solid}0D` }}>
              <p className="text-sm text-ink/70 dark:text-white/70 leading-relaxed">
                {ui.gastrointestinal.anatomyIntro}
              </p>
            </div>

            {structuresLoading && (
              <p className="text-sm text-ink/40 dark:text-white/40">{ui.gastrointestinal.loading}</p>
            )}
            ...
            {!structuresLoading && !structuresError && (
              <div className="space-y-8">
                {(['system_overview', 'organ'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      ...
                      >
                        {ui.gastrointestinal.categoryLabels[cat] ?? cat}
                      </h3>
        ...
        {subView === 'conditions' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.gastrointestinal.conditionsHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.gastrointestinal.conditionsHint}
            </p>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {conditions.filter(matchesCondition).map((c) => (
                  <button
                    ...
                    className="text-left rounded-2xl border border-black/[0.06] dark:border-white/10 bg-white/70 dark:bg-white/[0.03] backdrop-blur-xl p-5 transition-colors hover:border-[#F59E0B]/40"
                  >
        ...
        {subView === 'assessment' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.gastrointestinal.assessmentHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.gastrointestinal.assessmentHint}
            </p>
            ...
            {!testsLoading && !testsError && (
              <div className="space-y-8">
                {(['blood_panel', 'stool_marker', 'serology', 'functional_test', 'imaging', 'endoscopy'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.gastrointestinal.testCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((t) => (
                          ...
                          >
                            ...
                            {t.procedure && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed mb-2">
                                <span className="font-semibold">{ui.gastrointestinal.procedureLabel}: </span>{t.procedure}
                              </p>
                            )}
                            {t.interpretation && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed">
                                <span className="font-semibold">{ui.gastrointestinal.interpretationLabel}: </span>{t.interpretation}
                              </p>
                            )}
                          </div>
                        ))}
        ...
        {subView === 'rehab' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.gastrointestinal.rehabHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.gastrointestinal.rehabHint}
            </p>
            ...
            {!rehabLoading && !rehabError && (
              <div className="space-y-8">
                {(['gi_disease_management', 'chronic_disease_management', 'post_surgical', 'sports_nutrition'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.gastrointestinal.rehabCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((r) => (
                          <div
                            ...
                            {r.protocol && (
                              <div className="rounded-xl bg-black/[0.02] dark:bg-white/[0.03] p-3 mb-3">
                                <p className="text-[10px] font-semibold uppercase tracking-wide text-ink/50 dark:text-white/50 mb-1">
                                  {ui.gastrointestinal.protocolLabel}
                                </p>
                                <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed">
                                  {r.protocol}
                                </p>
        ...
        {selectedCondition && (
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                <ClinicalActionBar contentType="condition" contentId={String(selectedCondition.id)} label={selectedCondition.condition_name} section="Gastrointestinal" />
              </div>
