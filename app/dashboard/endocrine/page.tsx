...
const ACCENT = {
  gradient: 'linear-gradient(90deg, #10B981 0%, #0EA5E9 100%)',
  solid: '#10B981',
};
...
export default function EndocrinePage() {
  ...
  useEffect(() => {
    ...
    const fetchStructures = async () => {
      ...
      try {
        const res = await fetch(`/api/endocrine/structures?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle strutture');
        const data = await res.json();
        setStructures(data.structures ?? []);
        setHasFetchedStructures(true);
      } catch (err) {
        setStructuresError(ui.endocrine.errorLoadingStructures);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchConditions = async () => {
      ...
      try {
        const res = await fetch(`/api/endocrine/conditions?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle patologie');
        const data = await res.json();
        setConditions(data.conditions ?? []);
        setHasFetchedConditions(true);
      } catch (err) {
        setConditionsError(ui.endocrine.errorLoadingConditions);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchTests = async () => {
      ...
      setTestsError(null);
      try {
        const res = await fetch(`/api/endocrine/tests?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero dei test');
        const data = await res.json();
        setTests(data.tests ?? []);
        setHasFetchedTests(true);
      } catch (err) {
        setTestsError(ui.endocrine.errorLoadingTests);
        console.error(err);
      } finally {
  ...
  const SUB_TABS: { key: SubView; label: string }[] = [
    { key: 'anatomy', label: ui.endocrine.subTabs.anatomy },
    { key: 'conditions', label: ui.endocrine.subTabs.conditions },
    { key: 'assessment', label: ui.endocrine.subTabs.assessment },
    { key: 'rehab', label: ui.endocrine.subTabs.rehab },
  ];
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-[#08090b] text-ink dark:text-white overflow-hidden transition-colors">
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(16,185,129,0.6) 0%, rgba(14,165,233,0.5) 100%)',
        }}
      />
          ...
          <div>
            ...
            {!structuresLoading && !structuresError && (
              <div className="space-y-8">
                {(['gland', 'axis', 'concept'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      ...
                      >
                        {ui.endocrine.categoryLabels[cat] ?? cat}
                      </h3>
        ...
        {subView === 'assessment' && (
          <div>
            ...
            {!testsLoading && !testsError && (
              <div className="space-y-8">
                {(['hormonal', 'metabolic', 'structural'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.endocrine.testCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((t) => (
                          ...
                          >
                            ...
                            {t.procedure && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed mb-2">
                                <span className="font-semibold">{ui.endocrine.procedureLabel}: </span>{t.procedure}
                              </p>
                            )}
                            {t.interpretation && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed">
                                <span className="font-semibold">{ui.endocrine.interpretationLabel}: </span>{t.interpretation}
                              </p>
                            )}
                          </div>
                        ))}
        ...
        {subView === 'rehab' && (
          <div>
            ...
            {!rehabLoading && !rehabError && (
              <div className="space-y-8">
                {(['bone_health', 'fall_prevention', 'hormone_replacement', 'metabolic_training', 'nutritional_support'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.endocrine.rehabCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((r) => (
                          <div
        ...
        {selectedCondition && (
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                <ClinicalActionBar contentType="condition" contentId={String(selectedCondition.id)} label={selectedCondition.condition_name} section="Endocrine" />
              </div>
