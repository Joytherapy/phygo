...
export default function UrinaryPage() {
  ...
  useEffect(() => {
    ...
    const fetchStructures = async () => {
      ...
      try {
        const res = await fetch(`/api/urinary/structures?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle strutture');
        const data = await res.json();
        setStructures(data.structures ?? []);
        setHasFetchedStructures(true);
      } catch (err) {
        setStructuresError(ui.urinary.errorLoadingStructures);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchConditions = async () => {
      ...
      try {
        const res = await fetch(`/api/urinary/conditions?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle patologie');
        const data = await res.json();
        setConditions(data.conditions ?? []);
        setHasFetchedConditions(true);
      } catch (err) {
        setConditionsError(ui.urinary.errorLoadingConditions);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchTests = async () => {
      ...
      setTestsError(null);
      try {
        const res = await fetch(`/api/urinary/tests?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero dei test');
        const data = await res.json();
        setTests(data.tests ?? []);
        setHasFetchedTests(true);
      } catch (err) {
        setTestsError(ui.urinary.errorLoadingTests);
        console.error(err);
      } finally {
  ...
  const SUB_TABS: { key: SubView; label: string }[] = [
    { key: 'anatomy', label: ui.urinary.subTabs.anatomy },
    { key: 'conditions', label: ui.urinary.subTabs.conditions },
    { key: 'assessment', label: ui.urinary.subTabs.assessment },
    { key: 'rehab', label: ui.urinary.subTabs.rehab },
  ];
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-[#08090b] text-ink dark:text-white overflow-hidden transition-colors">
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(34,211,238,0.6) 0%, rgba(59,130,246,0.5) 100%)',
        }}
      />
      ...
      <div className="relative max-w-4xl mx-auto px-6 pt-40 pb-24">
          ...
          <div>
            ...
            <div className="mb-10 rounded-2xl border p-6" style={{ borderColor: `${ACCENT.solid}33`, background: `${ACCENT.solid}0D` }}>
              <p className="text-sm text-ink/70 dark:text-white/70 leading-relaxed">
                {ui.urinary.anatomyIntro}
              </p>
            </div>

            {structuresLoading && (
              <p className="text-sm text-ink/40 dark:text-white/40">{ui.urinary.loading}</p>
            )}
            ...
            {!structuresLoading && !structuresError && (
              <div className="space-y-8">
                {(['organ', 'physiology'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      ...
                      >
                        {ui.urinary.categoryLabels[cat] ?? cat}
                      </h3>
        ...
        {subView === 'conditions' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.urinary.conditionsHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.urinary.conditionsHint}
            </p>
        ...
        {subView === 'assessment' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.urinary.assessmentHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.urinary.assessmentHint}
            </p>
            ...
            {!testsLoading && !testsError && (
              <div className="space-y-8">
                {(['renal_function', 'urinalysis', 'metabolic', 'imaging'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.urinary.testCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((t) => (
                          ...
                          >
                            ...
                            {t.procedure && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed mb-2">
                                <span className="font-semibold">{ui.urinary.procedureLabel}: </span>{t.procedure}
                              </p>
                            )}
                            {t.interpretation && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed">
                                <span className="font-semibold">{ui.urinary.interpretationLabel}: </span>{t.interpretation}
                              </p>
                            )}
                          </div>
                        ))}
        ...
        {subView === 'rehab' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.urinary.rehabHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.urinary.rehabHint}
            </p>
            ...
            {!rehabLoading && !rehabError && (
              <div className="space-y-8">
                {(['renal_training', 'sports_nephrology'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.urinary.rehabCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((r) => (
                          <div
        ...
        {selectedCondition && (
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                <ClinicalActionBar contentType="condition" contentId={String(selectedCondition.id)} label={selectedCondition.condition_name} section="Urinary" />
              </div>
