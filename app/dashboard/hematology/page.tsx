...
export default function HematologyPage() {
  ...
  useEffect(() => {
    ...
    const fetchStructures = async () => {
      ...
      try {
        const res = await fetch(`/api/hematology/structures?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle strutture');
        const data = await res.json();
        setStructures(data.structures ?? []);
        setHasFetchedStructures(true);
      } catch (err) {
        setStructuresError(ui.hematology.errorLoadingStructures);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchConditions = async () => {
      ...
      try {
        const res = await fetch(`/api/hematology/conditions?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero delle patologie');
        const data = await res.json();
        setConditions(data.conditions ?? []);
        setHasFetchedConditions(true);
      } catch (err) {
        setConditionsError(ui.hematology.errorLoadingConditions);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchTests = async () => {
      ...
      setTestsError(null);
      try {
        const res = await fetch(`/api/hematology/tests?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero dei test');
        const data = await res.json();
        setTests(data.tests ?? []);
        setHasFetchedTests(true);
      } catch (err) {
        setTestsError(ui.hematology.errorLoadingTests);
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchRehab = async () => {
      ...
      setRehabError(null);
      try {
        const res = await fetch(`/api/hematology/rehab?lang=${lang}`);
        if (!res.ok) throw new Error('Errore nel recupero dei contenuti riabilitativi');
        const data = await res.json();
        setRehab(data.rehab ?? []);
        setHasFetchedRehab(true);
      } catch (err) {
        setRehabError(ui.hematology.errorLoadingRehab);
        console.error(err);
      } finally {
  ...
  const SUB_TABS: { key: SubView; label: string }[] = [
    { key: 'anatomy', label: ui.hematology.subTabs.anatomy },
    { key: 'conditions', label: ui.hematology.subTabs.conditions },
    { key: 'assessment', label: ui.hematology.subTabs.assessment },
    { key: 'rehab', label: ui.hematology.subTabs.rehab },
  ];
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-[#08090b] text-ink dark:text-white overflow-hidden transition-colors">
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(225,29,72,0.6) 0%, rgba(251,113,133,0.5) 100%)',
        }}
      />
      ...
      <div className="relative max-w-4xl mx-auto px-6 pt-40 pb-24">
          ...
          <div>
            ...
            <div className="mb-10 rounded-2xl border p-6" style={{ borderColor: `${ACCENT.solid}33`, background: `${ACCENT.solid}0D` }}>
              <p className="text-sm text-ink/70 dark:text-white/70 leading-relaxed">
                {ui.hematology.anatomyIntro}
              </p>
            </div>

            {structuresLoading && (
              <p className="text-sm text-ink/40 dark:text-white/40">{ui.hematology.loading}</p>
            )}
            ...
            {!structuresLoading && !structuresError && (
              <div className="space-y-8">
                {(['cell_line', 'fluid', 'molecule', 'process'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      ...
                      >
                        {ui.hematology.categoryLabels[cat] ?? cat}
                      </h3>
        ...
        {subView === 'conditions' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.hematology.conditionsHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.hematology.conditionsHint}
            </p>
            ...
            {!conditionsLoading && !conditionsError && (
              // Hematology conditions have no system sub-split (unlike Cardiopulmonary's
              // cardiac/respiratory/mixed_systemic) — hematology_condition_tags.system is
              // always 'hematology', so they are all shown in a single flat grid.
              <div className="grid sm:grid-cols-2 gap-3">
                {conditions.filter(matchesCondition).map((c) => (
                  <button
                    key={c.id}
                    onClick={() => setSelectedCondition(c)}
                    className="text-left rounded-2xl border border-black/[0.06] dark:border-white/10 bg-white/70 dark:bg-white/[0.03] backdrop-blur-xl p-5 transition-colors hover:border-[#E11D48]/40"
                  >
        ...
        {subView === 'assessment' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.hematology.assessmentHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.hematology.assessmentHint}
            </p>
            ...
            {!testsLoading && !testsError && (
              <div className="space-y-8">
                {(['coagulation', 'diagnostic', 'general', 'metabolic'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.hematology.testCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((t) => (
                          ...
                          >
                            ...
                            {t.procedure && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed mb-2">
                                <span className="font-semibold">{ui.hematology.procedureLabel}: </span>{t.procedure}
                              </p>
                            )}
                            {t.interpretation && (
                              <p className="text-xs text-ink/60 dark:text-white/60 leading-relaxed">
                                <span className="font-semibold">{ui.hematology.interpretationLabel}: </span>{t.interpretation}
                              </p>
                            )}
                          </div>
                        ))}
        ...
        {subView === 'rehab' && (
          <div>
            <h2 className="text-sm font-semibold tracking-wide uppercase text-ink/60 dark:text-white/60 mb-2">
              {ui.hematology.rehabHeading}
            </h2>
            <p className="text-sm text-ink/50 dark:text-white/50 mb-6 max-w-2xl">
              {ui.hematology.rehabHint}
            </p>
            ...
            {!rehabLoading && !rehabError && (
              <div className="space-y-8">
                {(['exercise_prescription', 'post_surgical', 'precaution_protocol', 'condition_specific'] as const).map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <h3
                        ...
                        style={{ color: ACCENT.solid }}
                      >
                        {ui.hematology.rehabCategoryLabels[cat] ?? cat}
                      </h3>
                      <div className="space-y-3">
                        {items.map((r) => (
                          <div
        ...
        {selectedCondition && (
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                <ClinicalActionBar contentType="condition" contentId={String(selectedCondition.id)} label={selectedCondition.condition_name} section="Hematology" />
              </div>
