...
import { REHAB_FIELD_ORDER } from '@/lib/urinaryFields';
...
export async function GET(req: Request) {
  try {
    ...
    const { data, error } = await adminSupabase
      .from('urinary_rehab')
      .select('*')
      .order('category', { ascending: true }).order('id', { ascending: true });

    if (error) {
      console.error('urinary rehab list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const rehab = await Promise.all(
      (data ?? []).map(async (r) => {
        const { fields } = await translateContent('urinary_rehab', r.id, r, REHAB_FIELD_ORDER, lang);
        return { ...r, ...fields };
      })
    );
    ...
    console.error('urinary rehab list error:', err);
