...
import { TEST_FIELD_ORDER } from '@/lib/urinaryFields';
...
export async function GET(req: Request) {
  try {
    ...
    if (error) {
      console.error('urinary tests list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const tests = await Promise.all(
      (data ?? []).map(async (t) => {
        const { fields } = await translateContent('urinary_test', t.id, t, TEST_FIELD_ORDER, lang);
        return { ...t, ...fields };
      })
    );
    ...
    console.error('urinary tests list error:', err);
