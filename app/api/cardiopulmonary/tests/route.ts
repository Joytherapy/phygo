...
import { TEST_FIELD_ORDER } from '@/lib/cardiopulmonaryFields';
...
export async function GET(req: Request) {
  try {
    ...
    const { data, error } = await adminSupabase
      .from('cardiopulmonary_tests')
      .select('*')
      .order('category', { ascending: true });

    if (error) {
      console.error('cardiopulmonary tests list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const tests = await Promise.all(
      (data ?? []).map(async (t) => {
        const { fields } = await translateContent('cardiopulmonary_test', t.id, t, TEST_FIELD_ORDER, lang);
        return { ...t, ...fields };
      })
    );
    ...
    console.error('cardiopulmonary tests list error:', err);
