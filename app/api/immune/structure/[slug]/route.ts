...
export async function GET(
  ...
  try {
    ...
    if (structureError || !structure) {
      console.error('immune structure detail error:', structureError);
    ...
    const { data: links, error: linksError } = await adminSupabase
      .from('immune_conditions')
      .select('condition_id')
      .eq('structure_id', structure.id);

    if (linksError) {
      console.error('immune structure conditions error:', linksError);
    ...
    if (conditionIds.length > 0) {
      if (lang === 'it') {
        ...
        if (conditionsError) {
          console.error('immune structure conditions fetch error:', conditionsError);
        } else {
    ...
    console.error('immune structure detail error:', err);
