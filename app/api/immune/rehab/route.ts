...
import { REHAB_FIELD_ORDER } from '@/lib/immuneFields';
...
export async function GET(req: Request) {
  try {
    ...
    if (error) {
      console.error('immune rehab list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const rehab = await Promise.all(
      (data ?? []).map(async (r) => {
        const { fields } = await translateContent('immune_rehab', r.id, r, REHAB_FIELD_ORDER, lang);
        return { ...r, ...fields };
      })
    );
    ...
    console.error('immune rehab list error:', err);
