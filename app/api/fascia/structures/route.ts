...
import { STRUCTURE_FIELD_ORDER } from '@/lib/fasciaFields';
...
export async function GET(req: Request) {
  try {
    ...
    const { data, error } = await adminSupabase
      .from('fascia_structures')
      .select('id,slug,name,category,anatomy,function,clinical_relevance,diagram_image,created_at')
      .order('category', { ascending: true });

    if (error) {
      console.error('fascia structures list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const structures = await Promise.all(
      (data ?? []).map(async (s) => {
        const { fields } = await translateContent('fascia_structure', s.id, s, STRUCTURE_FIELD_ORDER, lang);
        return { ...s, ...fields };
      })
    );
    ...
    console.error('fascia structures list error:', err);
