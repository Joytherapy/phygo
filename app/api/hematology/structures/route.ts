...
export async function GET(req: Request) {
  try {
    ...
    if (error) {
      console.error('hematology structures list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const structures = await Promise.all(
      (data ?? []).map(async (s) => {
        const { fields } = await translateContent('hematology_structure', s.id, s, STRUCTURE_FIELD_ORDER, lang);
        return { ...s, ...fields };
      })
    );
    ...
    console.error('hematology structures list error:', err);
