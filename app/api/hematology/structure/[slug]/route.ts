...
import { STRUCTURE_FIELD_ORDER } from '@/lib/hematologyFields';
...
export async function GET(
  ...
  try {
    ...
    const { data: structure, error: structureError } = await adminSupabase
      .from('hematology_structures')
      .select('*')
      .eq('slug', slug)
      .single();

    if (structureError || !structure) {
      console.error('hematology structure detail error:', structureError);
      return NextResponse.json({ error: 'Structure not found' }, { status: 404 });
    }

    const { fields } = await translateContent(
      'hematology_structure',
      structure.id,
      structure,
    ...
    const { data: links, error: linksError } = await adminSupabase
      .from('hematology_conditions')
      .select('condition_id')
      .eq('structure_id', structure.id);

    if (linksError) {
      console.error('hematology structure conditions error:', linksError);
    ...
    if (conditionIds.length > 0) {
      if (lang === 'it') {
        ...
        if (conditionsError) {
          console.error('hematology structure conditions fetch error:', conditionsError);
        } else {
    ...
    console.error('hematology structure detail error:', err);
