...
export async function GET(req: Request) {
  try {
    ...
    if (error) {
      console.error('endocrine structures list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const structures = await Promise.all(
      (data ?? []).map(async (s) => {
        const { fields } = await translateContent('endocrine_structure', s.id, s, STRUCTURE_FIELD_ORDER, lang);
        return { ...s, ...fields };
      })
    );
    ...
    console.error('endocrine structures list error:', err);
