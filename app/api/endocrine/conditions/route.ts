...
export async function GET(req: Request) {
  try {
    ...
    const { data: tags, error: tagsError } = await adminSupabase
      .from('endocrine_condition_tags')
      .select('condition_id, system');

    if (tagsError) {
      console.error('endocrine conditions tags error:', tagsError);
    ...
    if (conditionIds.length > 0) {
      if (lang === 'it') {
        ...
        if (error) {
          console.error('endocrine conditions fetch error:', error);
    ...
    // Every row in `endocrine_condition_tags` carries system = 'endocrine' —
    // there is no sub-system split in this section, unlike Cardiopulmonary.
    const merged = conditions.map((c) => ({ ...c, system: 'endocrine' as const }));

    return NextResponse.json({ conditions: merged });
  } catch (err) {
    console.error('endocrine conditions list error:', err);
