...
import { TEST_FIELD_ORDER } from '@/lib/endocrineFields';
...
export async function GET(req: Request) {
  try {
    ...
    const { data, error } = await adminSupabase
      .from('endocrine_tests')
      .select('*')
      .order('category', { ascending: true });

    if (error) {
      console.error('endocrine tests list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const tests = await Promise.all(
      (data ?? []).map(async (t) => {
        const { fields } = await translateContent('endocrine_test', t.id, t, TEST_FIELD_ORDER, lang);
        return { ...t, ...fields };
      })
    );
    ...
    console.error('endocrine tests list error:', err);
