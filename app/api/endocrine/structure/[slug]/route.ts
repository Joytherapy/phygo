...
import { STRUCTURE_FIELD_ORDER } from '@/lib/endocrineFields';
...
export async function GET(
  ...
  try {
    ...
    const { data: structure, error: structureError } = await adminSupabase
      .from('endocrine_structures')
      .select('*')
      .eq('slug', slug)
      .single();

    if (structureError || !structure) {
      console.error('endocrine structure detail error:', structureError);
    ...
    if (linksError) {
      console.error('endocrine structure conditions error:', linksError);
    ...
    if (conditionIds.length > 0) {
      if (lang === 'it') {
        ...
        if (conditionsError) {
          console.error('endocrine structure conditions fetch error:', conditionsError);
        } else {
    ...
    console.error('endocrine structure detail error:', err);
