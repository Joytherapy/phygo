...
import { REHAB_FIELD_ORDER } from '@/lib/gastrointestinalFields';
...
export async function GET(req: Request) {
  try {
    ...
    const { data, error } = await adminSupabase
      .from('gastrointestinal_rehab')
      .select('*')
      .order('category', { ascending: true });

    if (error) {
      console.error('gastrointestinal rehab list error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const rehab = await Promise.all(
      (data ?? []).map(async (r) => {
        const { fields } = await translateContent('gastrointestinal_rehab', r.id, r, REHAB_FIELD_ORDER, lang);
        return { ...r, ...fields };
      })
    );
    ...
    console.error('gastrointestinal rehab list error:', err);
