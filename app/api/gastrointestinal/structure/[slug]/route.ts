...
import { STRUCTURE_FIELD_ORDER } from '@/lib/gastrointestinalFields';
...
export async function GET(
  ...
  try {
    ...
    const { data: structure, error: structureError } = await adminSupabase
      .from('gastrointestinal_structures')
      .select('*')
      .eq('slug', slug)
      .single();

    if (structureError || !structure) {
      console.error('gastrointestinal structure detail error:', structureError);
      return NextResponse.json({ error: 'Structure not found' }, { status: 404 });
    }

    const { fields } = await translateContent(
      'gastrointestinal_structure',
      structure.id,
      structure,
    ...
    const { data: links, error: linksError } = await adminSupabase
      .from('gastrointestinal_conditions')
      .select('condition_id')
      .eq('structure_id', structure.id);

    if (linksError) {
      console.error('gastrointestinal structure conditions error:', linksError);
    ...
    if (conditionIds.length > 0) {
      if (lang === 'it') {
        ...
        if (conditionsError) {
          console.error('gastrointestinal structure conditions fetch error:', conditionsError);
        } else {
    ...
    console.error('gastrointestinal structure detail error:', err);
