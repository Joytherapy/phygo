...
export async function GET(req: Request) {
  try {
    ...
    const { data: tags, error: tagsError } = await adminSupabase
      .from('gastrointestinal_condition_tags')
      .select('condition_id');

    if (tagsError) {
      console.error('gastrointestinal conditions tags error:', tagsError);
    ...
    if (conditionIds.length > 0) {
      if (lang === 'it') {
        ...
        if (error) {
          console.error('gastrointestinal conditions fetch error:', error);
    ...
    // Unlike Cardiopulmonary, this section has a single sub-system
    // ('gastrointestinal') for every condition, so there is no grouping UI —
    // just tag every condition with the fixed system value for shape parity.
    const merged = conditions.map((c) => ({ ...c, system: 'gastrointestinal' as const }));

    return NextResponse.json({ conditions: merged });
  } catch (err) {
    console.error('gastrointestinal conditions list error:', err);
