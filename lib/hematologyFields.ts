// Stable field-order constants for Hematology content translated via the
...
// `hematology_condition_tags` bridge table), so they go through
// lib/conditionTranslation.ts's getTranslatedCondition() instead — not
// listed here. Structures, tests and rehab protocols each live in their
// own dedicated table, so they use this generic engine.
